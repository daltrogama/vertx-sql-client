...
public class DataObjectTypesTest extends PgTestBase {
  ...
  private <I, O> void testNumber(TestContext ctx, String sqlType, I value, O expected, String column, Function<TestDataObject, O> getter) {
    Async async = ctx.async();
    QueryTemplate<TestDataObject> template = QueryTemplate.create(connection, TestDataObjectMapper::fromRow, "SELECT :value :: " + sqlType + " \"" + column + "\"");
    template.query(Collections.singletonMap("value", value), ctx.asyncAssertSuccess(result -> {
      ctx.assertEquals(1, result.size());
      ctx.assertEquals(expected, getter.apply(result.get(0)));
      async.complete();
    }));
    async.await(10000);
  }
